...
router.put('/password', authenticate, async (req, res) => {
    ...
    try {
        ...
        if (!isValid) return res.status(400).json({ errorMessage: 'Invalid old password' })
        if (oldPassword === newPassword) return res.status(400).json({ errorMessage: 'New password cannot be the same as old password' })
